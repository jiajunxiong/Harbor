...
>
> MVP 3（样本外验证）进行中：样本外验证管线（冻结切分、参数搜索只用训练+验证、滚动 OOS、压力/覆盖门槛、结论 `QUALIFIED`/`NOT_QUALIFIED`/`INCONCLUSIVE` 可审计）与验证 CLI（`run`/`freeze`/`tune`/`evaluate`/`show`/`report`，SP 3.69–3.71）、示例验证配置（SP 3.72）与样本外方法与限制文档（SP 3.73）已落地。
