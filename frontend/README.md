...
`npm run dev` 只监听 `127.0.0.1`（Vite 的安全默认值）。需要从同一局域网的其它机器打开时：

```bash
npm run dev:host        # 等价于 vite --host 0.0.0.0，然后访问 http://<本机IP>:5173/
```

> ⚠️ **暴露到局域网前请先了解后果**：
>
> - `VITE_HARBOR_API_TOKEN` 会被内联进浏览器加载的 JS，因此**同网段的任何人都能读到该令牌并访问只读接口**。这是令牌必须为只读角色的原因；运维（ops）令牌绝不可这样暴露。
> - 只有看板端口（5173）需要暴露。**API 继续只监听 `127.0.0.1`**，局域网请求经 Vite 代理转发，因此 API 本身不直接对外。
> - 这仍然是一个开发服务器，不是生产部署；共享给不受信任的网络前请改用阶段 6 的容器化方案（SP 5.61/5.62）。

#### 如果局域网仍然连不上：检查主机防火墙

绑定 `0.0.0.0` 只解决"监听地址"，主机防火墙仍可能拦截。典型症状很具有迷惑性：**在服务器上 `curl http://<本机IP>:5173/` 正常，但别的机器超时** —— 因为发往本机地址的流量由本地投递，不经过 `INPUT` 链，所以本机自测**测不出**防火墙问题。

若启用了 ufw（且 `DEFAULT_INPUT_POLICY="DROP"`），需要显式放行（需 sudo，请自行在终端执行）：

```bash
sudo ufw allow from 192.168.0.0/24 to any port 5173 proto tcp   # 仅限本网段
```

注意 Docker 发布的端口（如 postgres 5433）会被 Docker **插入自己的 iptables 规则并绕过 ufw**，所以"5433 能连、5173 连不上"是常见现象，不代表 5173 的绑定有问题。API 的 8010 绑定在 `127.0.0.1`，**不需要**也不需要放行。
...
| 命令 | 说明 |
| :--- | :--- |
| `npm run dev` | 开发服务器（仅本机） |
| `npm run dev:host` | 开发服务器，监听 `0.0.0.0`，供局域网访问 |
